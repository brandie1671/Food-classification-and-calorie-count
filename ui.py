...
def fetch_nutrition(prediction, language='en', num_facts=1):
    try:
        url = f"https://www.google.com/search?q={prediction}+nutritients&hl={language}"
        req = requests.get(url).text
        scrap = BeautifulSoup(req, 'html.parser')
        nutrition_div = scrap.find_all("div", class_="BNeawe s3v9rd AP7Wnd")
        if nutrition_div:
            nutrition_facts = []
            for div in nutrition_div[:num_facts]:
                nutrition_facts.append(div.text)
            return nutrition_facts
        else:
            st.warning("Nutrition facts not found on Google Search results.")
    except Exception as e:
        st.error("Can't able to fetch the Nutrition Facts")
        print(e)
...
def run():
    st.markdown("<h1 style='text-align: center; color: #yellow; border-radius: 20px; background-color: gray;'>FOOD☕ CLASSIFICATION AND CALORIE COUNT✨</h1>", unsafe_allow_html=True)

    img_file = st.file_uploader("Choose an Image", type=["jpg", "png"])
    if img_file is not None:
        img = Image.open(img_file).resize((250, 250))
        st.image(img, use_column_width=False)
        save_image_path = os.path.abspath('./upload_images/' + img_file.name)
        ...
        if img_file is not None:
            ...
            nutrition = fetch_nutrition(result)
            if nutrition:
                st.info('**Nutrition Facts:**')
